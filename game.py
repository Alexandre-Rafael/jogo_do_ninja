...
class Game:
    def __init__(self):
        ...
        self._enemies = []
        self._level = self.save_progress.load_progress()
        self._transition = 0
        self._powerups = []
        self._particles = []
        self._sparks = []
        ...
        self._assets = {
            'decor': self.image_loader.load_images('tiles/decor'),
            'grass': self.image_loader.load_images('tiles/grass'),
            'large_decor': self.image_loader.load_images('tiles/large_decor'),
            'stone': self.image_loader.load_images('tiles/stone'),
            'player': self.image_loader.load_image('entities/player.png'),
            'background': self.image_loader.load_image('background.png'),
            ...
            'player/slide': Animation(self.image_loader.load_images('entities/player/slide')),
            'player/wall_slide': Animation(self.image_loader.load_images('entities/player/wall_slide')),
            'particle/leaf': Animation(self.image_loader.load_images('particles/leaf'), img_dur=20, loop=False),
            'particle/particle': Animation(self.image_loader.load_images('particles/particle'), img_dur=6, loop=False),
            'gun': self.image_loader.load_image('gun.png'),
            'projectile': self.image_loader.load_image('projectile.png'),
            'powerup': self.image_loader.load_image('powerup.png')
        }
        ...
        self._clouds = MultipleClouds(self._assets['clouds'], count=16)
            ...
            try:
                ...
                if self._transition:
                    ...
                    pygame.draw.circle(transition_surf, (255, 255, 255),
                                       (self._display.get_width() // 2, self._display.get_height() // 2),
                                       (30 - abs(self._transition)) * 8)
